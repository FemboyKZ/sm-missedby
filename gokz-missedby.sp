...
void SnapToNearbyWall(float pos[3])
{
	...
	for (int d = 0; d < 8; d++)
	{
		...
		Handle h = TR_TraceRayFilterEx(pos, end, MASK_SOLID, RayType_EndPoint,
			TraceEntityFilterPlayers);
		if (!TR_DidHit(h)) { delete h; continue; }

		float norm[3];
		TR_GetPlaneNormal(h, norm);
		if (FloatAbs(norm[2]) > 0.5) { delete h; continue; }  // floor/ceiling, not a wall
		...
		{
			bestDist = dist;
			bestNorm[0] = norm[0]; bestNorm[1] = norm[1]; bestNorm[2] = norm[2];
			bestHit[0]  = hitPt[0]; bestHit[1] = hitPt[1]; bestHit[2] = hitPt[2];
			found = true;
		}
	}
	...
	int seed, const int mouse[2])
{
	...
	for (int spec = 1; spec <= MaxClients; spec++)
	{
		...
		float sEO2[2];
		sEO2[0] = sEO[0] + 32.0; sEO2[1] = sEO[1];
		...
		float sOS[3], sOE[3];
		sOS[0] = sEO[0]; sOS[1] = sEO[1]; sOS[2] = sThresh;
		sOE[0] = sEO[0] - 32.0; sOE[1] = sEO[1]; sOE[2] = sThresh;
...
void FlashZone(int client, int zoneIdx)
{
	...
	p0[0] = g_fZoneP0[zoneIdx][0];  p0[1] = g_fZoneP0[zoneIdx][1];  p0[2] = drawZ;
	p1[0] = g_fZoneP1[zoneIdx][0];  p1[1] = g_fZoneP1[zoneIdx][1];  p1[2] = drawZ;
	p2[0] = g_fZoneP2[zoneIdx][0];  p2[1] = g_fZoneP2[zoneIdx][1];  p2[2] = drawZ;
...
void WsCrossDraw(int client)
{
	...
	{
		float a[3], b[3];
		a[0] = g_fWsPos3D[client][0][0]; a[1] = g_fWsPos3D[client][0][1];
		a[2] = (g_fWsPos3D[client][0][2] + g_fWsPos3D[client][1][2]) * 0.5;
		b[0] = g_fWsPos3D[client][1][0]; b[1] = g_fWsPos3D[client][1][1]; b[2] = a[2];
		MeasureBeam(client, a, b, WS_CROSS_LIFE, 0.15, 180, 180, 180);
	}
	if (g_bWsPosSet[client][2] && g_bWsPosSet[client][1])
	{
		float a[3], b[3];
		a[0] = g_fWsPos3D[client][2][0]; a[1] = g_fWsPos3D[client][2][1];
...
void WsCrossBeams(int client, float pos[3], float life, int r, int g, int b)
{
	...
	a0[0] = pos[0] + WS_CROSS_SIZE;  a0[1] = pos[1] + WS_CROSS_SIZE;  a0[2] = pos[2];
	a1[0] = pos[0] - WS_CROSS_SIZE;  a1[1] = pos[1] - WS_CROSS_SIZE;  a1[2] = pos[2];
	a2[0] = pos[0] + WS_CROSS_SIZE;  a2[1] = pos[1] - WS_CROSS_SIZE;  a2[2] = pos[2];
	a3[0] = pos[0] - WS_CROSS_SIZE;  a3[1] = pos[1] + WS_CROSS_SIZE;  a3[2] = pos[2];
